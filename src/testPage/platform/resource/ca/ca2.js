...
function init () {
	if(hasInited){
  	...
  	try{
		...
		// 16777220 "控件未安装" ;2952790019 "查询服务版本失败" ;4026531843
		// IE下"不匹配的CertKit版本"
...
function initUsbKeyCertList(){
	try{
		var keystore = CertStore.byName(name);
		if(keystore==null){
			// throw 指定的驱动未安装
		...
		//优化，列举秘钥错误：16777218
		if (e instanceof TCACErr && (e.number=="16777218")) {
  			window.location.href=exepath;
			console.log("initUsbKeyCertList() throws error:"+e.message);
  			return;
  		}	else{
			console.log("initUsbKeyCertList() throws error:"+e.message);
			throw e;
...
function signMessage(toSign, uid, flag) {
	...
	try {
		...
		if (e instanceof TCACErr && e.number == "2684354561") {
			...
			console.log("控件异常，请联系CA厂商。"+e.message);
			throw new Error("控件异常，请联系CA厂商。");
		} else {
			console.log("初始化未知异常，请联系CA厂商。"+e.message);
